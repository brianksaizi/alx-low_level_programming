...
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n)
{
	...
	new_node->n = n; /* adds our element to the new node*/

	/*iterates list to node position idx - 2*/
	...
	{
		new_node->next = current->next; /*places current node after new node*/
		current->next = new_node;/*sets the new node at index idx*/
	}
	else /*if node position is not present in the list*/
	{
		new_node->next = NULL;/*sets next addr as NULL, indicates last node*/
		current->next = new_node;/*sets the new node at the last position in list*/
	}
